...
class Carrier:
    ...
    def departure(self):
        ...
        self._distanceToStation = self._connections[self._movingFrom][self._movingTo].distance
    ...
    def stop_in(self, station):
        ...
        self._passengers = []
        yield self._env.process(self.idle(station.loader))

    def printDistance(self,tick):
        while True:
            print(f'{self._uid} decc: {self._breakingDistance:.3f},')
            print(f'{self._uid} to: {self._distanceToDestination:.3f},')
            print(f'{self._uid} left : {self._distanceLeftDestination:.3f},')
    ...
    def printEvents(self):
        ...
        while True:
            if self.state != preState:
                preState = self.state
                print(f'carrier {self._uid} {self.state.name} at {self._env.now},')
